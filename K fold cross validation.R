...
nrow(titanic_test)
nrow(titanic_test2_mean_lima)
titanic_test2_mean_lima <- titanic_test_start

#imputation by replacing NAs by means in the test set
titanic_test2_mean_lima$Age[is.na(titanic_test2_mean_lima$Age)] = mean(titanic_test2_mean_lima$Age, na.rm = TRUE)

titanic_test2_mean_lima$prob = predict(full.model.titanic.mean, newdata=titanic_test2_mean_lima, type=c("response"))
titanic_test2_mean_lima$Survived.pred = ifelse(titanic_test2_mean_lima$prob>=.5,'pred_yes','pred_no')
table(titanic_test2_mean_lima$Survived.pred,titanic_test2_mean_lima$Survived)
...
Kfoldobj.test <- Kfold_func(titanic_test2_mean_lima,Survived ~ Pclass + Sex + SibSp + Age,binomial,10)

#Calling the Mean Squared Error function on the test set here
MSE_Test <-MeanSquareError_func(titanic_test2_mean_lima,Survived ~ Pclass + Sex + SibSp + Age)

#Confusion matrix on test set
table(titanic_test2_mean_lima$Survived,round(Kfoldobj.test$cvhat))
print("Estimate of Accuracy")
print(Kfoldobj.test$acc.cv)

#"Mean squared error" - TRAIN SET ##
#[1] 0.1420293
# "Estimate of Accuracy" - TRAIN SET ##
#[1] 0.8022508

#"Mean squared error" - TEST SET ##
#[1] 0.1453662

#"Estimate of Accuracy" - TEST SET ##
#[1] 0.7843866
